...
class MetricaHelper(TraceHelper):
    def __init__(
        ...
        if traces_preprocessed is not None:
            ...
            coordinate_x_cols = ['start_x', 'end_x']
            coordinate_y_cols = ['start_y', 'end_y']
            events[coordinate_x_cols] *= pitch_size[0]
            events[coordinate_y_cols] *= pitch_size[1]
    ...
    def generate_phase_records(self):
        ...
        for i, f0 in enumerate(change_frames[:-1]):
            ...
            end_time = round(self.traces.at[f1 + 1, "time"] - 0.04, 1)
    ...
    #downsample없이 phase만 만드는 작업
    def generate_phase(self):
        self.traces["event_player"] = np.nan
        self.traces["event_type"] = np.nan
        ...
        traces = self.traces.set_index('time')
        traces['phase'] = 0
        ...
        for i in tqdm(self.events.index, desc="Combining tracking and event data"):
            ...
            traces.loc[t0:t1, "event_player"] = self.events.at[i, "from"]
            traces.loc[t0:t1, "event_type"] = self.events.at[i, "subtype"]

        for phase in self.phase_records.index:
            t0 = self.phase_records.at[phase, "start_time"]
            t1 = self.phase_records.at[phase, "end_time"]
            traces.loc[t0:t1, "phase"] = phase
            ...
            traces.loc[t0:t1, invalid_cols] = np.nan
            traces_interp = traces.loc[t0:t1, valid_cols].interpolate(limit_direction="both")
            traces.loc[t0:t1, valid_cols] = traces_interp

        traces["frame"] = np.arange(len(traces)) + 1
        traces["episode"] = 0
        traces["team_poss"] = np.nan
        traces["player_poss"] = np.nan
        traces[["ball_x", "ball_y"]] = (
            traces[["session", "ball_x", "ball_y"]]
            .groupby("session", group_keys=False)
            .apply(lambda x: x.interpolate(limit_direction="both"))[["ball_x", "ball_y"]]
        )

        self.traces = traces.reset_index()[meta_cols + xy_cols]
