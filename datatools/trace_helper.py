...
class TraceHelper:
    ...
    def calc_running_features(self, remove_outliers=True, smoothing=True):
        ...
        # 공의 MAX SPEED, MAX ACCEL기준은 선수랑 다르기 때문에 다르게 설정
        # if "ball_x" in self.traces.columns:
        #     data_cols += ["ball_x", "ball_y"]
        # self.traces[data_cols] = self.traces[data_cols].astype(float)
        self.calc_single_player_running_features("ball", remove_outliers, smoothing)
        data_cols += ["ball_x", "ball_y","ball_vx", "ball_vy","ball_speed", "ball_accel"]
